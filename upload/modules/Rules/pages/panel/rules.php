...
if (!isset($_GET["action"])) {
        ...
        if (Token::check(Input::get("token"))) {
            ...
            if ($validation->passed()) {
                try {
                    ...
                    $message_id = $queries->getWhere("rules_settings", [
                        "name",
                        "=",
                        "rules_message",
                    ]);
    ...
    if (count($rules_catagories)) {
        foreach ($rules_catagories as $catagory) {
            $catagories_array[] = [
                "edit_link" => URL::build(
                    "/panel/rules/",
                    "action=edit&id=" . Output::getClean($catagory->id)
                ),
                "name" => Output::getClean($catagory->name),
                "delete_link" => URL::build(
                    "/panel/rules/",
                    "action=delete&id=" . Output::getClean($catagory->id)
                ),
            ];
        }
    }
    ...
    if (count($rules_buttons)) {
        foreach ($rules_buttons as $button) {
            $buttons_array[] = [
                "edit_link" => URL::build(
                    "/panel/rules/",
                    "action=edit_button&id=" . Output::getClean($button->id)
                ),
                "name" => Output::getClean($button->name),
                "delete_link" => URL::build(
                    "/panel/rules/",
                    "action=delete_button&id=" . Output::getClean($button->id)
                ),
            ];
        }
    }
    ...
    $rules_message = $queries->getWhere("rules_settings", [
        "name",
        "=",
        "rules_message",
    ]);
    ...
    $smarty->assign([
        ...
        "RULES_BUTTON_NAME" => $rules_language->get(
            "rules",
            "rules_button_name"
        ),
        ...
        "CONFIRM_DELETE_BUTTON" => $rules_language->get(
            "rules",
            "delete_button"
        ),

        "NEW_CATAGORY" => $rules_language->get("rules", "new_catagory"),
        "RULES_CATAGORY_NAME" => $rules_language->get(
            "rules",
            "rules_catagory_name"
        ),
        ...
        "ICON_EXAMPLE" => htmlspecialchars(
            $rules_language->get("rules", "icon_example")
        ),
        "ICON_VALUE" => Output::getClean(htmlspecialchars_decode($icon)),
        "CATAGORY_LIST" => $catagories_array,
        "NO_RULES_CATAGORIES" => $rules_language->get(
            "rules",
            "no_rules_catagories"
        ),
        ...
        "CONFIRM_DELETE_CATAGORY" => $rules_language->get(
            "rules",
            "delete_catagory"
        ),
    ...
    switch ($_GET["action"]) {
        case "new":
                    ...
                    if ($validation->passed()) {
                        ...
                        try {
                            $queries->create("rules_catagories", [
                                "name" => htmlspecialchars(
                                    Input::get("rules_catagory_name")
                                ),
                                "icon" => htmlspecialchars(
                                    Input::get("rules_catagory_icon")
                                ),
                                "rules" => htmlspecialchars(
                                    Input::get("rules_catagory_rules")
                                ),
                            ]);
                            Session::flash(
                                "staff_rules",
                        ...
                        foreach ($validation->errors() as $item) {
                            if (strpos($item, "is required") !== false) {
                                if (
                                    strpos($item, "rules_catagory_name") !==
                                    false
                                ) {
                                    $errors[] = $rules_language->get(
                                        "rules",
                                        "catagory_name_required"
                                    );
                                } elseif (
                                    strpos($item, "rules_catagory_rules") !==
                                    false
                                ) {
                                    $errors[] = $rules_language->get(
                                        "rules",
                                        "catagory_rules_required"
                                    );
                                }
                                ...
                                if (
                                    ...
                                    $errors[] = $rules_language->get(
                                        "rules",
                                        "catagory_name_maximum"
                                    );
                                } elseif (
                                    strpos($item, "rules_catagory_icon") !==
                                    false
                                ) {
                                    $errors[] = $rules_language->get(
                                        "rules",
                                        "catagory_icon_maximum"
                                    );
                                }
                            }
                        }
            ...
            $smarty->assign([
                ...
                "RULES_CATAGORY_ICON" => $rules_language->get(
                    "rules",
                    "rules_catagory_icon"
                ),
                "RULES_CATAGORY_RULES" => $rules_language->get(
                    "rules",
                    "rules_catagory_rules"
                ),
            ]);
        ...
        case "edit":
            ...
            $catagory = $queries->getWhere("rules_catagories", [
                "id",
                "=",
                $_GET["id"],
            ]);
            ...
            $smarty->assign([
                "EDIT_CATAGORY" => $rules_language->get(
                    "rules",
                    "edit_catagory"
                ),
                "BACK" => $language->get("general", "back"),
                ...
                "RULES_CATAGORY_NAME_VALUE" => Output::getClean(
                    $catagory->name
                ),
                "RULES_CATAGORY_ICON" => $rules_language->get(
                    "rules",
                ...
                "RULES_CATAGORY_ICON_VALUE" => Output::getClean(
                    $catagory->icon
                ),
                "RULES_CATAGORY_RULES" => $rules_language->get(
                    "rules",
                ...
                "RULES_CATAGORY_RULES_VALUE" => Output::getClean(
                    $catagory->rules
                ),
            ]);
        ...
        case "delete":
            if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
                try {
                    $queries->delete("rules_catagories", [
                        "id",
                        "=",
                        $_GET["id"],
                    ]);
        ...
        case "new_button":
                    ...
                    if ($validation->passed()) {
                        ...
                        try {
                            $queries->create("rules_buttons", [
                                "name" => htmlspecialchars(
                                    Input::get("rules_button_name")
                                ),
                                "link" => htmlspecialchars(
                                    Input::get("rules_button_link")
                                ),
                            ]);
                            Session::flash(
                                "staff_rules",
                        ...
                        foreach ($validation->errors() as $item) {
                            if (strpos($item, "is required") !== false) {
                                if (
                                    strpos($item, "rules_button_name") !== false
                                ) {
                                    $errors[] = $rules_language->get(
                                        "rules",
                                    ...
                                    strpos($item, "rules_button_link") !== false
                                ) {
                                    $errors[] = $rules_language->get(
                                        "rules",
                                        "button_link_required"
                                    );
                                }
                            } elseif (strpos($item, "minimum") !== false) {
                                if (
            ...
            $smarty->assign([
                ...
                "RULES_BUTTON_LINK" => $rules_language->get(
                    "rules",
                    "rules_button_link"
                ),
            ]);
        ...
        case "edit_button":
            ...
            $button = $queries->getWhere("rules_buttons", [
                "id",
                "=",
                $_GET["id"],
            ]);
        ...
        case "delete_button":
            if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
                ...
                Session::flash(
                    "staff_rules",
                    $rules_language->get("rules", "button_deleted_successfully")
                );
                Redirect::to(URL::build("/panel/rules"));
                die();
            }
